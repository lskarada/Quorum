...
4. **Spend hard stop: $80.** Spend gate enforces $75 hard stop via `backend/scripts/spend_gate.sh`; do NOT raise without explicit user approval. Log spend after every API run with `bash backend/scripts/log_spend.sh AMOUNT "label"`.
