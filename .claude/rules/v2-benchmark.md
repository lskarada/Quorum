...
4. **Spend hard stop: $142.** Raised from $80 (gate $75) to **$142** by explicit user approval on 2026-05-30 for the v2.1 Accuracy-Maximization Campaign (~$100 incremental over the $42.35 already tracked). Spend gate enforces the $142 hard stop via `backend/scripts/spend_gate.sh`; do NOT raise further without explicit user approval. Log spend after every API run with `bash backend/scripts/log_spend.sh AMOUNT "label"`.
